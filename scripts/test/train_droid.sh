...
export DROID_DATA_ROOT="${DROID_DATA_ROOT:-/mnt/project_rlinf_hs/yuanhuining/datasets/DreamZero-DROID-Data}"
...
if [ ! -d "$DROID_DATA_ROOT/meta" ]; then
    for candidate in \
        /mnt/project_rlinf_hs/yuanhuining/datasets \
        /mnt/project_rlinf_hs/yuanhuining/datasets/DreamZero-DROID-Data \
        /mnt/project_rlinf_hs/yuanhuining/datasets/DreamZero-DROID-Data-mini
    do
        if [ -d "$candidate/meta" ]; then
            export DROID_DATA_ROOT="$candidate"
            break
        fi
    done
fi
...
LOG_FILE="$RUN_DIR/train_droid.log"

exec > >(tee -a "$LOG_FILE") 2>&1

cat > "$RUN_DIR/run_meta.txt" <<EOF
script=scripts/test/train_droid.sh
...
case "$TRAIN_ARCHITECTURE" in
    lora)
        TRAIN_SCRIPT="$PROJECT_ROOT/scripts/train/droid_training_lora.sh"
        ;;
    full|full_finetune)
        TRAIN_SCRIPT="$PROJECT_ROOT/scripts/train/droid_training_full_finetune.sh"
        ;;
    *)
